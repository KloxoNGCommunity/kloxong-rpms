...
Source0: http://www.php.net/distributions/%{realname}-%{version}_without-tests.tar.bz2
Source1: php.conf
Source2: php.ini
Source3: macros.php
...
# libxml 2.9.x patch
Patch2000: libxml29_compat.patch

# All backported patches and security fixes.
Patch3000: http://php52-backports.googlecode.com/files/php52-backports-20130717.patch

# litespeed patch
Patch3001: litespeed.patch
...
Provides: %{name}-spl, %{name}-sysvsem, %{name}-sysvshm, %{name}-sysvmsg, %{name}-tokenizer
Provides: %{name}-wddx, %{name}-zlib, %{name}-json, %{name}-zip, %{name}-dbase
...
Provides: php_database, %{name}-mysqli
...
%patch3000 -p 1 -b .patched -F 2
%patch3001 -p1 -b .litespeed
...
mkdir build-cgi build-apache build-embedded build-zts build-litespeed build-fpm
...
# Build fpm module
pushd build-fpm
build --with-fpm --with-libevent=shared,%{_prefix} ${without_shared}
popd
...
install -m 755 build-litespeed/sapi/litespeed/php $RPM_BUILD_ROOT%{_bindir}/php-ls

# install the php fpm binary
install -m 755 -d $RPM_BUILD_ROOT%{_sbindir}
install -m 755 build-fpm/sapi/fpm/php-fpm $RPM_BUILD_ROOT%{_sbindir}/php-fpm
install -m 644 build-fpm/sapi/fpm/php_fpm.conf $RPM_BUILD_ROOT%{_sysconfdir}/php-fpm.conf
install -m 755 -d $RPM_BUILD_ROOT%{_initrddir}
install -m 755 build-fpm/sapi/fpm/init.d.php_fpm $RPM_BUILD_ROOT%{_initrddir}/php-fpm
...
%changelog
* Wed Jun 04 2014 Mustafa Ramadhan <mustafa@bigraf.com> - 5.2.17-102
- enable php-fpm

* Tue Mar 18 2014 Mustafa Ramadhan <mustafa@bigraf.com> - 5.2.17-101
- disable to provides to apiver and zendver

* Sun Mar 2 2014 Mustafa Ramadhan <mustafa@bigraf.com> - 5.2.17-100
- rename to php52 for multi-php in Kloxo-MR
- release as 100 to make sure most newest compare to other rpms
