...
int addDomain(char *d) 
{
	...
	for (i = 0; i < DomainsTab.nval; i++)
		if (strcmp(DomainsTab.alldomains[i].domain, d) == 0)
			return -1;
...
void checkUser(char *dmn, char *m, int d, double byte)
{
	...
	tmpnval = -1;
	for (i = 0; i < DomainsTab.nval; i++) 
		if (strcmp(DomainsTab.alldomains[i].domain, dmn) == 0) {
			tmpnval = i;
			break;
		}
...
void sortDomainUsersFrom(char *domain) 
{
	...
	tmpnval = -1;
	for (i = 0; i < DomainsTab.nval; i++) 
		if (strcmp(DomainsTab.alldomains[i].domain, domain) == 0) {
			tmpnval = i;
			break;
		}
