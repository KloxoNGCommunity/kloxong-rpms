...
Source0: http://www.php.net/distributions/%{real_name}-%{version}_mod.tar.bz2
Source1: php.conf
Source2: php53-ius.ini
Source3: macros.php

Source4: php53s-fpm.conf
Source5: php53s-fpm-www.conf
Source6: php53s-fpm.init
Source7: php53s-fpm.logrotate
Source8: php53s-fpm.sysconfig

Source100: php53s.ini
Source101: php53s-cgi.sh
Source102: php53s-cli.sh
...
# add for php
Provides: %{name}-bz2, %{name}-calendar, %{name}-ctype
Provides: %{name}-curl, %{name}-date, %{name}-exif 
Provides: %{name}-ftp, %{name}-gettext, %{name}-gmp
Provides: %{name}-hash, %{name}-iconv, %{name}-libxml
Provides: %{name}-openssl, %{name}-pcre, 
Provides: %{name}-posix, %{name}-reflection
Provides: %{name}-session, %{name}-shmop, %{name}-simplexml
Provides: %{name}-sockets, %{name}-spl, %{name}-sysvsem
Provides: %{name}-sysvshm, %{name}-sysvmsg, %{name}-tokenizer
Provides: %{name}-wddx, %{name}-zlib, %{name}-json
Provides: %{name}-zip
Provides: %{name}-sqlite3
Provides: %{name}-fileinfo
...
Provides: %{name}-pdo-abi = %{pdover}
Provides: %{name}-pdo_sqlite = %{pdover}
...
Provides: php_database, %{name}-mysqli, %{name}-mysqli
...
Requires: %{name}-common = %{version}-%{release}
Provides: %{name}-milter = %{version}-%{release}
BuildRequires: sendmail-devel
...
Group: Development/Languages
Requires: %{name}-common = %{version}-%{release}
Provides: %{name}-oci8 = %{version}-%{release}
...
%configure \
        ...
        --with-libdir=%{_lib} \
        --with-config-file-path=%{_customdir}%{_sysconfdir} \
        --with-config-file-scan-dir=%{_customdir}%{_sysconfdir}/php.d \
        --disable-debug \
        --with-pic \
        --disable-rpath \
        --without-pear \
...
build --with-apxs2=%{_bindir}/apxs ${without_shared} \
      ...
      --with-config-file-scan-dir=%{_customdir}%{_sysconfdir}/php-zts.d
popd
%endif
...
mkdir -p %{buildroot}%{_customdir}

%if 0%{?_with_fpm}
# Install the php-fpm binary
make -C build-fpm install-fpm INSTALL_ROOT=%{buildroot}%{_customdir}
%endif

# Install the version for milter SAPI
%if 0%{?_with_milter}
make -C build-milter install-sapi install-headers INSTALL_ROOT=%{buildroot}%{_customdir}
%endif

# Install the version for embedded script language in applications + php_embed.h
%if 0%{?_with_embedded}
make -C build-embedded install-sapi install-headers INSTALL_ROOT=%{buildroot}%{_customdir}
%endif

# Install everything from the CGI SAPI build
pushd build-cgi
make install INSTALL_ROOT=%{buildroot}%{_customdir}
popd

# Install the Apache module
pushd build-apache
make install-sapi INSTALL_ROOT=%{buildroot}%{_customdir}
popd

# Install the default configuration file and icons
install -m 755 -d %{buildroot}%{_customdir}%{_sysconfdir}/
install -m 644 %{SOURCE2} %{buildroot}%{_customdir}%{_sysconfdir}/php53-ius.ini
install -m 644 %{SOURCE100} %{buildroot}%{_customdir}%{_sysconfdir}/php53s.ini
install -m 644 %{SOURCE100} %{buildroot}%{_customdir}%{_sysconfdir}/php.ini
install -m 755 -d %{buildroot}%{_customdir}%{contentdir}/icons
install -m 644 *.gif %{buildroot}%{_customdir}%{contentdir}/icons/

mkdir -p %{buildroot}%{_customdir}%{_sysconfdir}/rpm

# Use correct libdir
sed -i -e 's|/usr/lib|%{_customdir}%{_libdir}|' %{buildroot}%{_customdir}%{_sysconfdir}/php.ini

# install the DSO
install -m 755 -d %{buildroot}%{_customdir}%{_libdir}/httpd/modules
install -m 755 build-apache/libs/libphp5.so %{buildroot}%{_customdir}%{_libdir}/httpd/modules

%if 0%{?_with_zts:1}
# install the ZTS DSO
install -m 755 build-zts/libs/libphp5.so %{buildroot}%{_customdir}%{_libdir}/httpd/modules/libphp5-zts.so
%endif

%if 0%{?_with_litespeed:1}
# install the php litespeed binary
install -m 755 build-litespeed/sapi/litespeed/php %{buildroot}%{_customdir}%{_bindir}/php-ls
%endif

# Apache config fragment
install -m 755 -d %{buildroot}%{_customdir}/etc/httpd/conf.d
install -m 644 %{SOURCE1} %{buildroot}%{_customdir}/etc/httpd/conf.d

install -m 755 -d %{buildroot}%{_customdir}%{_sysconfdir}/php.d
install -m 755 -d %{buildroot}%{_customdir}%{_localstatedir}/lib/php
install -m 700 -d %{buildroot}%{_customdir}%{_localstatedir}/lib/php/session

%if 0%{?_with_fpm}
# PHP-FPM stuff
# Log
install -m 755 -d %{buildroot}%{_customdir}%{_localstatedir}/log/php-fpm
install -m 755 -d %{buildroot}%{_customdir}%{_localstatedir}/run/php-fpm
# Config
install -m 755 -d %{buildroot}%{_customdir}%{_sysconfdir}/php-fpm.d
install -m 644 %{SOURCE4} %{buildroot}%{_customdir}%{_sysconfdir}/php-fpm.conf
install -m 644 %{SOURCE5} %{buildroot}%{_customdir}%{_sysconfdir}/php-fpm.d/www.conf
mv %{buildroot}%{_customdir}%{_sysconfdir}/php-fpm.conf.default .
# Service
install -m 755 -d %{buildroot}%{_customdir}%{_initrddir}
install -m 755 %{SOURCE6} %{buildroot}%{_customdir}%{_initrddir}/php-fpm
# LogRotate
install -m 755 -d %{buildroot}%{_customdir}%{_sysconfdir}/logrotate.d
install -m 644 %{SOURCE7} %{buildroot}%{_customdir}%{_sysconfdir}/logrotate.d/php-fpm
# Sysconfig
install -m 755 -d %{buildroot}%{_customdir}%{_sysconfdir}/sysconfig
install -m 644 %{SOURCE8} %{buildroot}%{_customdir}%{_sysconfdir}/sysconfig/php-fpm
# Move php-fpm binary
#mv %{buildroot}%{_customdir}%{_sbindir}/php-fpm %{buildroot}%{_customdir}%{_bindir}/php-fpm
%endif
...
for mod in pgsql mysql mysqli odbc ldap snmp xmlrpc imap \
    ...
    cat > %{buildroot}%{_customdir}%{_sysconfdir}/php.d/${mod}.ini <<EOF
; Enable ${mod} extension module
extension=${mod}.so
EOF
    cat > files.${mod} <<EOF
%attr(755,root,root) %{_customdir}%{_libdir}/php/modules/${mod}.so
%config(noreplace) %attr(644,root,root) %{_customdir}%{_sysconfdir}/php.d/${mod}.ini
EOF
done
...
install -m 644 -c macros.php \
           %{buildroot}%{_customdir}%{_sysconfdir}/rpm/macros.php


# Remove unpackaged files
rm -rf %{buildroot}%{_customdir}%{_libdir}/php/modules/*.a \
       %{buildroot}%{_customdir}%{_bindir}/{phptar} \
       %{buildroot}%{_customdir}%{_datadir}/pear \
       %{buildroot}%{_customdir}%{_libdir}/libphp5.la \
	%{buildroot}%{_customdir}%{_customdir}

# Remove irrelevant docs
rm -f README.{Zeus,QNX,CVS-RULES}

# Fix the link
(cd %{buildroot}%{_customdir}%{_bindir}; ln -sfn phar.phar phar)

# Patch php.ini extension_dir
if [ "%{_libdir}" == "/usr/lib64" ] ; then
	sed -i -e "s|%{_customdir}/usr/lib/php/modules|%{_customdir}%{_libdir}/php/modules|" \
		 %{buildroot}%{_customdir}%{_sysconfdir}/php53-ius.ini
	sed -i -e "s|%{_customdir}/usr/lib/php/modules|%{_customdir}%{_libdir}/php/modules|" \
		%{buildroot}%{_customdir}%{_sysconfdir}/php53s.ini
	sed -i -e "s|%{_customdir}/usr/lib/php/modules|%{_customdir}%{_libdir}/php/modules|" \
		%{buildroot}%{_customdir}%{_sysconfdir}/php.ini
fi
...
%post
if [ "%{_libdir}" == "/usr/lib64" ] ; then
	ln -sf %{_customdir}%{_libdir} %{_customdir}/usr/lib
fi

%postun
if [ "%{_libdir}" == "/usr/lib64" ] ; then
	rm -rf %{_customdir}/usr/lib
fi

%if 0%{?_with_milter}
%post milter -p /sbin/ldconfig
milter -p /sbin/ldconfig
%endif
...
%files
%defattr(-,root,root)
%{_customdir}%{_libdir}/httpd/modules/libphp5.so
%attr(0770,root,apache) %dir %{_customdir}%{_localstatedir}/lib/php/session
%config %{_customdir}%{_sysconfdir}/httpd/conf.d/php.conf
%{_customdir}%{contentdir}/icons/php.gif

%files common -f files.common
%defattr(-,root,root)
%doc CODING_STANDARDS CREDITS EXTENSIONS INSTALL LICENSE NEWS README*
%doc Zend/ZEND_* TSRM_LICENSE 
%config(noreplace) %{_customdir}%{_sysconfdir}/php.ini
%config(noreplace) %{_customdir}%{_sysconfdir}/php53-ius.ini
%config(noreplace) %{_customdir}%{_sysconfdir}/php53s.ini
%dir %{_customdir}%{_sysconfdir}/php.d
%dir %{_customdir}%{_libdir}/php
%dir %{_customdir}%{_libdir}/php/modules
%dir %{_customdir}%{_localstatedir}/lib/php

%files cli
%defattr(-,root,root)
%doc sapi/cgi/README* sapi/cli/README
%{_customdir}%{_bindir}/php
%{_customdir}%{_bindir}/phar*
%{_customdir}%{_bindir}/php-cgi
%{_customdir}%{_mandir}/man1/php.1*
# provides phpize here (not in -devel) for pecl command
%{_customdir}%{_bindir}/phpize
%{_customdir}%{_mandir}/man1/phpize.1*

%files devel
%defattr(-,root,root)
%{_customdir}%{_bindir}/php-config
%{_customdir}%{_includedir}/php
%{_customdir}%{_libdir}/php/build
%{_customdir}%{_mandir}/man1/php-config.1*
%config %{_customdir}%{_sysconfdir}/rpm/macros.php

%if 0%{?_with_fpm}
%files fpm
%defattr(-,root,root)
%doc php-fpm.conf.default
%config(noreplace) %{_customdir}%{_sysconfdir}/php-fpm.conf
%config(noreplace) %{_customdir}%{_sysconfdir}/php-fpm.d/www.conf
%config(noreplace) %{_customdir}%{_sysconfdir}/logrotate.d/php-fpm
%config(noreplace) %{_customdir}%{_sysconfdir}/sysconfig/php-fpm
%{_customdir}%{_sbindir}/php-fpm
%{_customdir}%{_initrddir}/php-fpm
%dir %{_customdir}%{_sysconfdir}/php-fpm.d
# log owned by apache for log
%attr(770,apache,apache) %dir %{_customdir}%{_localstatedir}/log/php-fpm
%dir %{_customdir}%{_localstatedir}/run/php-fpm
%{_customdir}%{_mandir}/man8/php-fpm.8
%{_customdir}%{_datadir}/fpm/status.html
%endif
...
%files zts
%defattr(-,root,root)
%{_customdir}%{_libdir}/httpd/modules/libphp5-zts.so
%endif

%if 0%{?_with_litespeed}
%files litespeed
%defattr(-,root,root)
%{_customdir}%{_bindir}/php-ls
%endif

%if 0%{?_with_milter}
%files milter
%defattr(-,root,root)
%{_customdir}%{_bindir}/php-milter
%endif

%if 0%{?_with_embedded}
%files embedded
%defattr(-,root,root,-)
%{_customdir}%{_libdir}/libphp5.so
%{_customdir}%{_libdir}/libphp5-%{version}.so
%endif
...
%changelog
* Sun Mar 23 2014 Mustafa Ramadhan <mustafa@bigraf.com> - 5.3.28-4.mr
- fix fpm (disable post and preun because special php)

* Sun Feb 16 2014 Mustafa Ramadhan <mustafa@bigraf.com> - 5.3.28-3.mr
- remove 'provides' to mod_php (escape for conflict with other then php 5.3)

* Thu Feb 13 2014 Mustafa Ramadhan <mustafa@bigraf.com> - 5.3.28-2.mr
- also include php53-ius.ini php53s.ini
- add extension_dir in php*.ini

* Thu Feb 6 2014 Mustafa Ramadhan <mustafa@bigraf.com> - 5.3.28-1.mr
- first compile of php53s (use /opt/php53s as directory)
- remove tests dirs in source0 tar file
- disable provides to php53-
...
- Remove 'Requires: php53s' (base package) as the -common subpackage
  does not necessarily need base... and php53s-cli doesn't need the 
...
- Add '#Provides: php53s-sqlite3' under -common sub package. 
  Resolves LP#722648
...
- Name change to php53s based on the fact that RHEL 5.6 includes 
