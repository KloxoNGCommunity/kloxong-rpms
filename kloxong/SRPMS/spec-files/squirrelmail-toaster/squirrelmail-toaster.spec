...
%define		builddate Tue Apr 06 2010
...
Source0:        squirrelmail-%{pversion}.tar.bz2
Source1:	quota_usage-1.3.1-1.2.7.tar.bz2
Source2:	unsafe_image_rules.0.8-1.4.tar.bz2
Source3:	config_local.php
Source4:	notes.1.2-1.4.0.tar.bz2
Source5:	qmailadmin_login-1.1-1.4.3.tar.bz2
Patch0:		squirrelmail-toaster-02052009.patch.bz2
BuildRoot:      %{_tmppath}/%{name}-%{version}-buildroot
BuildArch:      noarch
Requires:       perl, aspell
...
# install plugins
tar xvfj %{SOURCE1}
mkdir plugins/quota_usage
cp -fR quota_usage/* plugins/quota_usage/

tar xvfj %{SOURCE2}
mkdir plugins/unsafe_image_rules
cp -fR unsafe_image_rules/* plugins/unsafe_image_rules/

tar xvfj %{SOURCE4}
mkdir plugins/notes
cp -fR notes/* plugins/notes/

tar xvfj %{SOURCE5}
mkdir plugins/qmailadmin_login
cp -fR qmailadmin_login/* plugins/qmailadmin_login/

%{__rm} -f plugins/make_archive.pl

# Rearrange the documentation
## Next 2 lines commented out on May 13, 2009 for the 1.3.18 release
## The files are already in the doc/ directory, so there is no need to move them
#%{__mv} doc/AUTHORS doc/ChangeLog doc/COPYING doc/INSTALL doc/README doc/UPGRADE doc/
...
mv -f %{buildroot}%{lcdir}/config_local.php %{buildroot}%{lcdir}/config_local.php.dist
install -m644 %{SOURCE3} %{buildroot}%{lcdir}/config_local.php
...
%{__mkdir_p} %{buildroot}%{apachedir}

cat <<EOF > squirrelmail.conf
...
%{__install} -m 644 squirrelmail.conf \
%{buildroot}%{apachedir}
...
%changelog
#------------------------------------------------------------------------------
* Sat Dec 20 2014 Mustafa Ramadhan <mustafa@bigraf.com> 1.4.20-1.3.18.mr
- cleanup spec based on toaster github (without define like build_cnt_60)

* Sat Jan 12 2013 Mustafa Ramadhan <mustafa@bigraf.com> 1.4.20-1.3.17.mr
