...
%install
rm -rf %{buildroot}
%{__mkdir} -p %{buildroot}/%{kloxocp}/%{packagename}
%{__cp} -ad ./* %{buildroot}/%{kloxocp}/%{packagename}
%{__cp} CONFIG %{buildroot}/%{kloxocp}/%{packagename}/config.inc.php

%{__rm} -rf %{buildroot}/%{kloxocp}/%{packagename}/setup
%{__rm} -rf %{buildroot}/%{kloxocp}/%{packagename}/contrib
...
#    %{kloxocp}/%{packagename}/config.inc.php

sed -i "s:// \$cfg['Servers'][\$i]['central_columns']:\$cfg['Servers'][\$i]['central_columns']:g" %{kloxocp}/%{packagename}/config.inc.php
rm -rf %{kloxocp}/%{packagename}/config.inc.php.*


%files
%defattr(-,lxlabs,lxlabs,-)
%{kloxocp}/%{packagename}
%dir %{kloxocp}/%{packagename}
#%config(noreplace) %{kloxocp}/%{packagename}/config.inc.php
...
* Wed Feb 19 2014 Mustafa Ramadhan <mustafa@bigraf.com> - 4.1.7-1.mr
- update to 4.1.7 (not compatible with php 5.2)

* Tue Feb 4 2014 Mustafa Ramadhan <mustafa@bigraf.com> - 4.1.6-1.mr
- update to 4.1.6 (not compatible with php 5.2)
...
* Mon Sep 2 2013 Mustafa Ramadhan <mustafa@bigraf.com> - 4.0.5-1.mr
- as cp panel
