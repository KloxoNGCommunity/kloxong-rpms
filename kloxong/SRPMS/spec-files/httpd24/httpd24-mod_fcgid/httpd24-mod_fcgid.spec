...
%{!?_httpd_apxs: %global _httpd_apxs /opt/rh/httpd24/root/usr/bin/apxs}
...
Patch10:  mod_fcgid-2.3.6-fix.patch
BuildRoot:	%{_tmppath}/%{name}-%{version}-%{release}-root-%(%{__id_u} -n)
BuildRequires:	httpd24-httpd-devel, pkgconfig, httpd24-apr-devel, httpd24-apr-util-devel
...
echo "LoadModule fcgid_module modules/mod_fcgid.so" > %{buildroot}/opt/rh/httpd24/root/%{_httpd_modconfdir}/10-fcgid.conf
install -D -m 644 fcgid24.conf %{buildroot}/opt/rh/httpd24/root/%{_httpd_confdir}/fcgid.conf
%else
install -D -m 644 fcgid.conf %{buildroot}/opt/rh/httpd24/root/%{_httpd_confdir}/fcgid.conf
%endif
install -d -m 755 %{buildroot}%{rundir}/httpd24-mod_fcgid
...
rm -rf %{buildroot}/opt/rh/httpd24/root/usr/share/httpd/manual/mod/*
...
/sbin/restorecon -R %{rundir}/httpd24-mod_fcgid || :
...
if [ $1 -eq 0 ]; then
	...
	[ -d %{rundir}/httpd24-mod_fcgid ] && \
		/sbin/restorecon -R %{rundir}/httpd24-mod_fcgid &> /dev/null || :
fi
exit 0
%endif
...
/opt/rh/httpd24/root/%{_libdir}/httpd/modules/mod_fcgid.so
%if %{httpd24}
%config(noreplace) /opt/rh/httpd24/root/%{_httpd_modconfdir}/10-fcgid.conf
%endif
%config(noreplace) /opt/rh/httpd24/root/%{_httpd_confdir}/fcgid.conf
%if 0%{?fedora} > 14 || 0%{?rhel} > 6
%{_prefix}/lib/tmpfiles.d/mod_fcgid.conf
%endif
%dir %attr(0755,apache,apache) %{rundir}/httpd24-mod_fcgid/
...
%changelog
* Thu Oct 10 2013 CloudLinux Packaging Team <packager@cloudlinux.com> 2.3.9-1
- Fixed killing of system processes when memory not enough on fork
