...
void openlogdir(char *logdir)
{
	...
	while((dirp = readdir(dp)) != NULL)  /* we are opening directory to find today's first created file */
		if ((dirp->d_name[0] == '@')) {
			if ((timeconvert(dirp->d_name, cur_year, cur_month, cur_day, &fdate)) > 0 ) {
				printf("today file: %s  ", dirp->d_name);
				printf(" date %d : \n", fdate);
				if(state == 0) {
					ffirst = fdate;
	...
	while( (dirp = readdir(dp)) != NULL) 
		if (dirp->d_name[0] == '@'){
			if ((timeconvert(dirp->d_name, cur_year, cur_month, cur_day, &fdate)) > 0 ) {
				if ((strcmp(fname, dirp->d_name)) != 0) /* if this file is not our firts created file*/
...
int timeconvert(char *p, int year, int month, int day, int *fdate)
{
 int  c;
...
void readSpecialLogFile(char *fn, int year, int month, int day)
{
	...
	while ((fgets(buf, 1024, fp)) != NULL)
		if ((timeconvert(buf, year, month, day, &i)) > 0) {
